...
define( 'LIMBENET_THEME_VERSION', '0.1.2' );

/**
 * Set up theme supports.
 */
function limbenet_setup() {
	load_theme_textdomain( 'limbenet-festivaltrail', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'primary' => __( 'Primary Navigation', 'limbenet-festivaltrail' ),
			'footer'  => __( 'Footer Navigation', 'limbenet-festivaltrail' ),
		)
	);
}
...
function limbenet_register_pattern_categories() {
	register_block_pattern_category(
		'limbenet-layout',
		array( 'label' => __( 'Limbe.Net Layouts', 'limbenet-festivaltrail' ) )
	);

	register_block_pattern_category(
		'limbenet-sections',
		array( 'label' => __( 'Limbe.Net Sections', 'limbenet-festivaltrail' ) )
	);
}
